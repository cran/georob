...
% hunspell -d en_GB -t -i mac  georob_vignette.Rnw
...
\SweaveOpts{echo=TRUE, eval=TRUE}      % <<- Anpassen!!
...
\newcommand{\mb}[1]{\mbox{\boldmath\ensuremath{#1}\unboldmath}}
\newcommand{\mbsubscript}[1]{\mbox{\boldmath\({\scriptstyle#1}\)\unboldmath}}
...
% material taken from
% tools::Rd2latex("georob-package.Rd", "georob-package.tex", outputEncoding="macintosh")
...
sections~\ref{sec:est-details} and \ref{sec:krig-details}.
...
We use the following model for the data \eqn{y_{i}=y(\mbox{\boldmath$s$\unboldmath}_{i})}{}:
%
\begin{equation}\label{eq:model}
  Y(\mbox{\boldmath$s$\unboldmath}_{i}) = 
  Z(\mbox{\boldmath$s$\unboldmath}_{i}) + \varepsilon_i =
  \mbox{\boldmath $x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}_{i})^\mathrm{T}
  \mbox{\boldmath$\beta$\unboldmath} +
  B(\mbox{\boldmath$s$\unboldmath}_{i}) +
  \varepsilon_i,
\end{equation}
%
where \eqn{\mbox{\boldmath$s$\unboldmath}_{i}}{} denotes a data
location, \eqn{Z(\mbox{\boldmath$s$\unboldmath}_{i})=\mbox{\boldmath
$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}_{i})^\mathrm{T}
\mbox{\boldmath$\beta$\unboldmath} +
B(\mbox{\boldmath$s$\unboldmath}_{i})}{} is the so-called signal,
\eqn{\mbox{\boldmath
$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}_{i})^\mathrm{T}
\mbox{\boldmath$\beta$\unboldmath}}{} is the external drift,
\eqn{\{B(\mbox{\boldmath$s$\unboldmath})\}}{} is an unobserved
...
\begin{equation}\label{eq:model-vector}
  \mbox{\boldmath $Y = X \beta + B + \varepsilon$\unboldmath},
\end{equation}
%
where \eqn{\mbox{\boldmath $X$\unboldmath}}{} is the model matrix with
the rows \eqn{\mbox{\boldmath
$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}_i)^\mathrm{T}}{}.
...
\begin{equation}\label{eq:covariance-matrix-signal}
  ...
  \sigma^2_Z \, (\xi \, \mbox{\boldmath$I$\unboldmath} + 
  (1-\xi)\, \mbox{\boldmath$V$\unboldmath}_\alpha ),
\end{equation}
%
where 
\eqn{\sigma_{\mathrm{n}}^2}{} 
is the variance of seemingly uncorrelated micro-scale variation in 
\eqn{B(\mbox{\boldmath$s$\unboldmath})}{} 
...
\eqn{B(\mbox{\boldmath$s$\unboldmath})}{},  
\eqn{\sigma_Z^2=\sigma_{\mathrm{n}}^2+\sigma^2}{} 
is the signal variance, and 
\eqn{\xi=\sigma_{\mathrm{n}}^2/\sigma_Z^2}{}. 
To estimate both 
...
We define \eqn{\mbox{\boldmath$V$\unboldmath}_{\alpha}}{} to be the
matrix with elements
%
\begin{equation}\label{eq:valpha}
  (\mbox{\boldmath$V$\unboldmath}_{\alpha})_{ij} =
...
\eqn{\mbox{\boldmath$V$\unboldmath}_{\alpha}}{} is positive definite,
\eqn{\gamma(\cdot)}{} is a valid stationary or intrinsic variogram,
and \eqn{\mbox{\boldmath$A$\unboldmath}
% =
% \mbox{\boldmath$A$\unboldmath}(\alpha, f_1, f_2; \omega, \phi,
% \zeta)
}{} is a matrix that is used to model geometrically anisotropic
auto-correlation (see section~\ref{sec:est-details-anisotropy}).
...
\eqn{\mbox{\boldmath $Y$\unboldmath}}{} is given by
%
\begin{equation}\label{eq:covariance-matrix-response}
  \mathrm{Cov}[\mbox{\boldmath $Y$\unboldmath},\mbox{\boldmath $Y$\unboldmath}^\mathrm{T}]
...
\eqn{\mbox{\boldmath$\theta$\unboldmath}}{} is used to denote all
...
\eqn{\mbox{\boldmath$B$\unboldmath}}{} at the data locations 
\eqn{\mbox{\boldmath$s$\unboldmath}_i}{}
and the model parameters 
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{}, $\tau^2$ and
...
given \eqn{\mbox{\boldmath$\theta$\unboldmath}}{} and \eqn{\tau^2}{} the
kriging predictions (= BLUP) of
\eqn{\mbox{\boldmath$B$\unboldmath}}{} and the generalized least
squares (GLS = ML) estimates of
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{} can be obtained
...
%
with respect to
\eqn{\mbox{\boldmath$B$\unboldmath}}{} and
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{}, e.g.
% \Cite{Harville (1977)}.
\citet{Harville-1977}.

Hence, the BLUP of \eqn{\mbox{\boldmath$B$\unboldmath}}{},
ML estimates of \eqn{\mbox{\boldmath$\beta$\unboldmath}}{},  
\eqn{\mbox{\boldmath$\theta$\unboldmath}}{} and \eqn{\tau^2}{} 
...
\end{equation}
%
jointly with respect to \eqn{\mbox{\boldmath$B$\unboldmath}}{},
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{},
\eqn{\mbox{\boldmath$\theta$\unboldmath}}{} and  \eqn{\tau^2}{}  or by solving the
...
(IRWLS) to estimate \eqn{\mbox{\boldmath$B$\unboldmath}}{} and
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{} for given 
\eqn{\mbox{\boldmath$\theta$\unboldmath}}{} and  \eqn{\tau^2}{} 
and non-linear root finding by the function 
\code{nleqslv()} of the \R{} package \pkg{nleqslv}
to get \eqn{\mbox{\boldmath$\theta$\unboldmath}}{} and  \eqn{\tau^2}{}.
...
\pkg{RandomFields} (see \code{RMmodel()}).  For most variogram
parameters, closed-form expressions of \eqn{\partial \gamma/ \partial
\theta_i}{} are used in the computations.  However, for the parameter
\eqn{\nu}{} of the models \code{"RMbessel"}, \code{"RMmatern"} and
\code{"RMwhittle"} \eqn{\partial \gamma/ \partial \nu}{} is evaluated
numerically by the function \code{numericDeriv()}, and this results in
an increase in computing time when \eqn{\nu}{} is estimated.
...
\eqn{\mbox{\boldmath$s$\unboldmath}_0}{}
from the covariates
\eqn{\mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}_0)}{},
the estimated parameters 
\eqn{\widehat{\mbox{\boldmath$\beta$\unboldmath}}}{},
\eqn{\widehat{\mbox{\boldmath$\theta$\unboldmath}}}{}
and the predicted random effects
\eqn{\widehat{\mbox{\boldmath$B$\unboldmath}}}{}
by
%
...
%
where
\eqn{\mbox{\boldmath$\Gamma$\unboldmath}_{\widehat{\theta}}}{}
...
\begin{enumerate}
  ...
  predictions (see \code{predict.georob()},
...
<<meuse-zinc-load,eval=TRUE,echo=FALSE,eval=FALSE>>=
...
along with a number of soil and landscape variable at the observation
...
@
%
\ecf{Dependence of \code{zinc} on \code{dist}ance to river, frequency
of flooding (\code{ffreq}) and \code{soil} type.}{meuse-eda-plot-2}
%
...
%
\ecf{Dependence of \code{zinc} concentration on \code{dist}ance to
river, frequency of flooding (\code{ffreq}) and \code{soil} type.}{meuse-eda-plot-3}
%
...
direction-indepdendent sample variogram and fit a spherical variogram
model by weighted non-linear least squares (using Cressie's weights)
%
\bcf
%
<<meuse-zinc-lm-res-sv-2,fig=TRUE,width=6,height=4,results=hide,eval=TRUE>>=
library(georob)
...
successful.
%
Nevertheless, before we interprete the output, we compute the profile
...
r.prfl.m0.spher.reml.scale <- profilelogLik(r.georob.m0.spher.reml,
  values=data.frame(scale=seq(500, 5000, by=50)), ncores=1)
@
%
\bcf
%
...
% section~\ref{sec:control-georob}.  The maximized restricted
...
r.cv.m0.spher.reml <- cv(r.georob.m0.spher.reml, seed=3245, lgn=TRUE, ncores=1)
r.georob.m1.spher.reml <- update(r.georob.m0.spher.reml, .~.-ffreq)
r.cv.m1.spher.reml <- cv(r.georob.m1.spher.reml, seed=3245, lgn=TRUE, ncores=1)
@
%
...
\code{log(zinc)\~{}sqrt(dist)+ffreq} (magenta).}{meuse-zinc-cv-plot}
...
score \code{crps}), but it models prediction uncertainty better
\citep[PIT closer to uniform distribution, see section~\ref{sec:cv}
and][]{Gneiting-etal-2007}.  
...
by REML (blue), ML (magenta) and fit to sample variogram
(darkgreen).}{meuse-zinc-georob-plot}
%
...
\begin{itemize}
  ...
  the back-transformation are computed.
...
back-transformation proposed by \citet{Cressie-2006} (see
section~\ref{eq:spatial-covariance-covariates}):
%
...
\begin{itemize}
  ...
  section~\ref{sec:block-kriging}).
  
  \item \code{mmax} controls parallelization when computing kriging
  predictions, see
  section~\ref{sec:kriging-parallelized-computations}.
  ...
  \citep[see equation~\ref{eq:spatial-covariance-covariates}
...
back-transform them as before under the permanence of log-normality
assumption:
%
...
@
%
Both the predictions and the prediction standard errors differ
somewhat for \code{block1}.  Based on the \citeauthor{Cressie-2006}'s
...
<<meuse-zinc-cleanup-2,echo=FALSE,eval=FALSE>>=
...
<<ash-load,eval=TRUE,echo=FALSE,eval=FALSE>>=
...
\bcf
%
<<ash-diag-lmrob,fig=TRUE,width=6,height=5,eval=TRUE>>=
op <- par(mfrow=c(2,2))
...
($\mathrm{Q_{n}}$-estimate).}{coalash-anisotropic-samplevariogram-lmrob}
%
...
$t$-distribution are also implemented (see section~\ref{sec:control-georob}):
%
<<ash-georob-robust-1>>=
...
\code{control.nleqslv()}, see section~\ref{sec:control-georob}.
%
...
plot(r.georob.m1.exp.c2, lag.dist.def=1, max.lag=10, estimator="qn", add = TRUE, 
  col="blue")
legend("bottomright", lt=1, col=c("black","blue"), 
  legend =c("Gaussian REML (c=1000)", "robust REML (c=2)"), bty="n")
@
%
...
% \eqn{\xi=\sigma_{\mathrm{n}}^2/\sigma_Z^2}{}, \eqn{\eta = \sigma_Z^2 /
% \tau^2} and $\alpha$ with respect to these parameters, and the
% variance of the signal
% \eqn{\sigma_Z^2=\sigma_{\mathrm{n}}^2+\sigma^2}{} is estimated by an
% explicit expression that depends on these parameters
% \citep[e.g.][p.~113]{Diggle-Ribeiro-2007}, see help page of
% \code{georob()}.
%
The outliers inflate mostly the nugget (by $\approx$ 20~\%) and less
so the signal variance and range parameter (by $\approx$ 10~\%).  The
estimated drift coefficients hardly differ.
...
section~\ref{sec:est-details-initial-values}).  Sometimes it helps
...
section~\ref{sec:est-details-initial-values}):

<<ash-cv-georob-gaussian-robust-1,eval=FALSE>>=
r.cv.georob.m1.exp.c2 <- cv(r.georob.m1.exp.c2, seed=1, param=r.georob.m1.exp.c2$param,
...
r.cv.georob.m1.exp.c2 <- cv(r.georob.m1.exp.c2, seed=1, param=r.georob.m1.exp.c2$param,
  control=control.georob(initial.param=FALSE), ncores=1)
r.cv.georob.m1.exp.c1000 <- cv(r.georob.m1.exp.c1000, seed=1, ncores=1)
@
% %
% <<ash-results-save-3,echo=FALSE>>=
% <<ash-results-save-1>>
...
\code{method} of \code{cv.georob()} and section~\ref{sec:cv}):
%
\bcf[0.55]
%
...
\code{cv.georob()}.}{coalash-cv-devault-partition}
...
\citep[see section~\ref{sec:cv} and][for more details about the
...
\code{predict.georob()}.  \code{newdata} can be an customary
...
r.pk.m1.exp.c2 <- predict(r.georob.m1.exp.c2, newdata=coalash.grid, control=control.predict.georob(ncores=1))
r.pk.m1.exp.c1000 <- predict(r.georob.m1.exp.c1000, newdata=coalash.grid, control=control.predict.georob(ncores=1))
@
% %
% <<ash-results-save-4,echo=FALSE>>=
% <<ash-results-save-1>>
% @
% %
...
and section~\ref{sec:prediction-targets}).  Apart from kriging
...
REML.}{coalash-diff-kriging-gaussian-robusts}
%
...
details]{Hofer-Papritz-2011}.  The third argument
...
@
%
<<ash-rm,eval=TRUE,echo=FALSE,eval=FALSE>>=
rm(list=ls(pattern="^r\\."))
@
...
% material taken from
% tools::Rd2latex("georob.Rd", "../../../vignettes/georob.tex", outputEncoding="macintosh")
% tools::Rd2latex("control.georob.Rd", "../../../vignettes/control.georob.tex", outputEncoding="macintosh")
...
as used by the functions \code{RM...()} of the package
\pkg{RandomFields} (see \code{RMmodel())}.  Use the
function \code{param.names()} to list additional parameters of a
...
of semi-principal axes, \eqn{\mbox{\boldmath$p$\unboldmath}_1}{},
\eqn{\mbox{\boldmath$p$\unboldmath}_2}{},
\eqn{\mbox{\boldmath$p$\unboldmath}_3}{}, equal to
\eqn{|\mbox{\boldmath$p$\unboldmath}_1|=\alpha}{},
\eqn{|\mbox{\boldmath$p$\unboldmath}_2|=f_1\,\alpha}{} and
\eqn{|\mbox{\boldmath$p$\unboldmath}_3|=f_2\,\alpha}{}, \eqn{0 < f_2
...
\begin{description}
  ...
  \item[\eqn{\omega}{}] is the azimuth of \eqn{\mbox{\boldmath$p$\unboldmath}_1}{}
  (= angle between north and the projection  
  of
  \eqn{\mbox{\boldmath$p$\unboldmath}_1}{}
  ...
  \eqn{\mbox{\boldmath$p$\unboldmath}_1}{} running through the origin
...
\begin{equation}\label{eq:coordinate-transformation-matrix}
  ...
  0 & 1/(f_1\,\alpha) & 0\\
  0 & 0 & 1/(f_2\,\alpha) \\
  \end{array}\right) ( \mbox{\boldmath$C$\unboldmath}_1,
  \mbox{\boldmath$C$\unboldmath}_2, \mbox{\boldmath$C$\unboldmath}_3)
\end{equation}
%
where
%
\begin{eqnarray}\label{eq:coordinate-rotation-matrix}
  \mbox{\boldmath$C$\unboldmath}_1^\mathrm{T} & = & (\sin\phi \sin\omega, -\cos\zeta \cos\omega + \sin\zeta \cos\phi \sin\omega, 
    -\cos\omega \sin\zeta - \cos\zeta \cos\phi \sin\omega) \nonumber \\
  \mbox{\boldmath$C$\unboldmath}_2^\mathrm{T} & = & (-\sin\phi \cos\omega, \cos\omega \sin\zeta \cos\phi 
    + \cos\zeta \sin\omega, -\cos\zeta \cos\omega \cos\phi + \sin\zeta \sin\omega)\nonumber \\
  \mbox{\boldmath$C$\unboldmath}_3^\mathrm{T} & = & (\cos\phi, -\sin\phi \sin\zeta, \cos\zeta \sin\phi)
\end{eqnarray}
%
...
\subsection{Estimating variance of micro-scale variation}\label{sec:est-details-microscale}

Simultaneous estimation of the variance of the micro-scale variation
(\code{snugget}, \eqn{\sigma_\mathrm{n}^2}{}), which appears seemingly
...
\eqn{\mbox{\boldmath$s$\unboldmath}_i}{} replicated observations are
...
\eqn{\sigma_Z^2 = \sigma^2 + \sigma_{\mathrm{n}}^2}{},

\item the inverse relative nugget 
\eqn{\eta = \sigma_Z^2 / \tau^2}{} and

\item the relative spatial nugget 
\eqn{\xi = \sigma_{\mathrm{n}}^2/\sigma_Z^2}{}.
...
\eqn{\alpha}{}, \ldots, and \eqn{\sigma_Z^2}{} is estimated by an
explicit expression that depends on these parameters
...
(see \code{param.bounds()} and \code{RMmodel()} for allowed ranges).
...
\begin{enumerate}
  ...
  section~\ref{sec:parameter-transformations} for detailed
...
\eqn{\mbox{\boldmath$B$\unboldmath}}{} and
\eqn{\mbox{\boldmath$\beta$\unboldmath}}{} the following initial
estimates are used:
...
\begin{itemize}
  ...
  \item \eqn{\widehat{\mbox{\boldmath$\beta$\unboldmath}}}{} is either
...
% covariances of \eqn{\widehat{\mbox{\boldmath$B$\unboldmath}}}{}.
% These covariances (and the covariances of
% \eqn{\mbox{\boldmath$B$\unboldmath}-\widehat{\mbox{\boldmath$B$\unboldmath}}}{},
% \eqn{\widehat{\mbox{\boldmath$\beta$\unboldmath}}}{},
% \eqn{\widehat{\mbox{\boldmath$\varepsilon$\unboldmath}}}{},
% \eqn{\widehat{\mbox{\boldmath$\varepsilon$\unboldmath}} +
% \widehat{\mbox{\boldmath$B$\unboldmath}}}{}) are approximated by
...
%   \widehat{\mbox{\boldmath$\beta$\unboldmath}} -
%   \widehat{\mbox{\boldmath$B$\unboldmath}}}{} and \eqn{\widehat{\mbox{\boldmath$\varepsilon$\unboldmath}} +
%   \widehat{\mbox{\boldmath$B$\unboldmath}}
%   =\mbox{\boldmath$Y$\unboldmath} - \mbox{\boldmath$X$\unboldmath}
%   \widehat{\mbox{\boldmath$\beta$\unboldmath}}}{} (\code{error.family.cov.residuals}).
%   
% \end{itemize}
% 
...
% tools::Rd2latex("control.georob.Rd", "../../../vignettes/control.georob.tex", outputEncoding="macintosh")
% tools::Rd2latex("pmm.Rd", "../../../vignettes/pmm.tex", outputEncoding="macintosh")
...
variogram parameters are estimated (see section~\ref{sec:est-details-gaussian-reml}).
%
...
\code{TRUE}, see section~\ref{sec:est-details-initial-values}).
...
\begin{ldescription}
  ...
  \eqn{\widehat{\mbox{\boldmath$B$\unboldmath}}}{}, with
  \eqn{\widehat{\mbox{\boldmath$B$\unboldmath}}=\mbox{\boldmath$0$\unboldmath}}{}
  ...
  \eqn{\mbox{\boldmath$X$\unboldmath}}{} (default \code{1.e-12}).
...
\begin{enumerate}
  ...
  \code{profilelogLik()} (section~\ref{sec:model-assessment}),
  \code{add1.georob()}, \code{drop1.georob()} and
  \code{step.georob()} (section~\ref{sec:model-building});
 
  \item \label{item:parallel-predict} computing kriging predictions by
  \code{predict.georob()} (section~\ref{sec:predict-georob});
...
\item[\code{f}] number (integer, default 2) of tasks assigned to each core in
...
time.  A sensible default for the number of cores for tasks
\ref{item:parallel-pmm} and \ref{item:parallel-gcr} is likely
2.  \code{max.ncores} controls how many child processes
are generated in total.  This can be used to prevent that child
processes generate themselves children which may result in too many
child processes.
...
% material taken from
% tools::Rd2latex("predict.georob.Rd", "../../../vignettes/predict.georob.tex", outputEncoding="macintosh")
% tools::Rd2latex("lgnpp.Rd", "../../../vignettes/lgnpp.tex", outputEncoding="macintosh")
% tools::Rd2latex("validate.predictions.Rd", "../../../vignettes/validate.predictions.tex", outputEncoding="macintosh")
...
\strong{SpatialPixels} or a \strong{SpatialGrid} object, the all the
latter provided by the package \pkg{sp}.  If \code{newdata} is a
\code{SpatialPoints}, \code{SpatialPixels} or a
\code{SpatialGrid} then the drift model may only use the coordinates
as covariates (universal kriging).  If \code{newdata} is a
...
section~\ref{sec:krig-details-lognormal}).  In more detail, the
following items are computed:

\begin{itemize}
  
  \item \code{trend}: the fitted values,
  \eqn{\mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath})\mathrm{^T}\widehat{\mbox{\boldmath$\beta$\unboldmath}}}{},
  
  \item \code{var.pred}: the variances of the kriging predictions,
  \eqn{\mathrm{Var}_{\hat{\theta}}[\widehat{Y}(\mbox{\boldmath$s$\unboldmath})]}{}
  or
  \eqn{\mathrm{Var}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath})]}{},
  
  \item \code{cov.pred.target}: the covariances between the
  predictions and the prediction targets,\\{}
  \eqn{\mathrm{Cov}_{\hat{\theta}}[\widehat{Y}(\mbox{\boldmath$s$\unboldmath}),Y(\mbox{\boldmath$s$\unboldmath})]}{}
  or
  \eqn{\mathrm{Cov}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath}),Z(\mbox{\boldmath$s$\unboldmath})]}{},
  
  
  \item \code{var.target}: the variances of the prediction targets
  \eqn{\mathrm{Var}_{\hat{\theta}}[Y(\mbox{\boldmath$s$\unboldmath})]}{}
  or
  \eqn{\mathrm{Var}_{\hat{\theta}}[Z(\mbox{\boldmath$s$\unboldmath})]}{}.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    prediction errors,
    \eqn{Y(\mbox{\boldmath$s$\unboldmath})-\widehat{Y}(\mbox{\boldmath$s$\unboldmath})}{}
    or
    \eqn{Z(\mbox{\boldmath$s$\unboldmath})-\widehat{Z}(\mbox{\boldmath$s$\unboldmath})}{}
    see above.
...
uniformed distributed in a rectangle is available in closed form
\citep{Hofer-Papritz-2011}.
%
...
function \code{control.pcmp()}, see section~\ref{sec:parallization}):
% 
% \begin{ldescription}
%   
...
returned by \code{{detectCores()}}.  Note that if \code{full.covmat} is
...
section~\ref{sec:predict-georob}.
...
%   
%   \item \code{var.pred}: the variances of the kriging predictions, 
%   \eqn{\mathrm{Var}_{\hat{\theta}}[\widehat{Y}(\mbox{\boldmath$s$\unboldmath})]}{} or 
%   \eqn{\mathrm{Var}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath})]}{},
%   
%   \item \code{cov.pred.target}: the covariances between the predictions and the
%   prediction targets,\\{}
%   \eqn{\mathrm{Cov}_{\hat{\theta}}[\widehat{Y}(\mbox{\boldmath$s$\unboldmath}),Y(\mbox{\boldmath$s$\unboldmath})]}{} or 
%   \eqn{\mathrm{Cov}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath}),Z(\mbox{\boldmath$s$\unboldmath})]}{},
%   
%   
%   \item \code{var.target}: the variances of the prediction targets
%   \eqn{\mathrm{Var}_{\hat{\theta}}[Y(\mbox{\boldmath$s$\unboldmath})]}{} or 
%   \eqn{\mathrm{Var}_{\hat{\theta}}[Z(\mbox{\boldmath$s$\unboldmath})]}{}.
%   
%   
% \end{itemize}
...
\begin{equation}\label{eq:point-lk-predictor}
  \widehat{U}(\mbox{\boldmath$s$\unboldmath}) = \exp( \widehat{Z}(\mbox{\boldmath$s$\unboldmath}) + 
  1/2 (  \mathrm{Var}_{\hat{\theta}}[ Z(\mbox{\boldmath$s$\unboldmath})]
...
\begin{eqnarray}\label{eq:point-lk-msep}
  ...
  U(\mbox{\boldmath$s$\unboldmath}_i) - \widehat{U}(\mbox{\boldmath$s$\unboldmath}_i),
  U(\mbox{\boldmath$s$\unboldmath}_j) - \widehat{U}(\mbox{\boldmath$s$\unboldmath}_j) 
  ] = \mu_{\hat{\theta}}(\mbox{\boldmath$s$\unboldmath}_i) \mu_{\hat{\theta}}(\mbox{\boldmath$s$\unboldmath}_j)
  \big\{
  \exp(\mathrm{Cov}_{\hat{\theta}}[Z(\mbox{\boldmath$s$\unboldmath}_i),Z(\mbox{\boldmath$s$\unboldmath}_j)])} \hspace{2cm}\nonumber \\
   &&  \quad\quad 
   -2\exp(\mathrm{Cov}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath}_i),Z(\mbox{\boldmath$s$\unboldmath}_j)]) 
    +\exp(\mathrm{Cov}_{\hat{\theta}}[\widehat{Z}(\mbox{\boldmath$s$\unboldmath}_i),\widehat{Z}(\mbox{\boldmath$s$\unboldmath}_j)])
  \big\},
\end{eqnarray}
...
\begin{equation}\label{eq:point-lk-mean}
  \mu_{\hat{\theta}}(\mbox{\boldmath$s$\unboldmath}) \approx 
      \exp(\mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath})\mathrm{^T}\widehat{\mbox{\boldmath$\beta$\unboldmath}} 
      + 1/2 \mathrm{Var}_{\hat{\theta}}[Z(\mbox{\boldmath$s$\unboldmath})]).    
\end{equation}
%
...
%
Instead of the signal \eqn{Z(\mbox{\boldmath$s$\unboldmath})}{},
predictions of the log-transformed response
\eqn{Y(\mbox{\boldmath$s$\unboldmath})}{} or the estimated trend
\eqn{\mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath})^\mathrm{T}\widehat{\mbox{\boldmath$\beta$\unboldmath}}}{}
...
\begin{equation}\label{eq:block-lk-predictor}
  \widehat{U}(A) = \exp( \widehat{Z}(A) + 1/2 \{
    \mathrm{Var}_{\hat{\theta}}[Z(\mbox{\boldmath$s$\unboldmath})] + \widehat{\mbox{\boldmath$\beta$\unboldmath}}\mathrm{^T}
    \mbox{\boldmath$M$\unboldmath}(A) \widehat{\mbox{\boldmath$\beta$\unboldmath}} -
...
\eqn{\mbox{\boldmath$M$\unboldmath}(A)}{} is the spatial
covariance matrix of the covariates

\begin{equation}  \label{eq:spatial-covariance-covariates}
\mbox{\boldmath$M$\unboldmath}(A) = 1/|A| \int_A 
  ( \mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}) - \mbox{\boldmath$x$\unboldmath}(A) )
  ( \mbox{\boldmath$x$\unboldmath}(\mbox{\boldmath$s$\unboldmath}) - \mbox{\boldmath$x$\unboldmath}(A) )\mathrm{^T} \,d\mbox{\boldmath$s$\unboldmath}
\end{equation}
...
\begin{equation}  \label{eq:spatial-mean}
  \mu_{\hat{\theta}}(A) \approx \exp(\mbox{\boldmath$x$\unboldmath}(A)\mathrm{^T}
    \widehat{\mbox{\boldmath$\beta$\unboldmath}} + 1/2 \mathrm{Var}_{\hat{\theta}}[Z(A)]). 
\end{equation}

This back-transformation is based on the assumption that both the
point data \eqn{U(\mbox{\boldmath$s$\unboldmath})}{} and the block
...
approximate \eqn{\mbox{\boldmath$M$\unboldmath}(A)}{M(A)}{}, one needs
...
\eqn{\mbox{\boldmath$M$\unboldmath}(A)}{} the geometry of the blocks
...
\begin{itemize}
  ...
  \begin{eqnarray}\label{eq:discrete-approx-msep-block-mean-2}
    ...
    \frac{1}{K^2} \sum_{s_i \in A} \mathrm{E}_{\hat{\theta}}[ \{ U(\mbox{\boldmath$s$\unboldmath}_i) - \widehat{U}(\mbox{\boldmath$s$\unboldmath}_i)\}^2]}  \nonumber\\
  ...
  \widehat{U}(\mbox{\boldmath$s$\unboldmath}_i)
  \end{equation}
  ...
  U(\mbox{\boldmath$s$\unboldmath}_i) -
  \widehat{U}(\mbox{\boldmath$s$\unboldmath}_i)\}^2].
  \end{equation}
...
% material taken from
% tools::Rd2latex("georobModelBuilding.Rd", "../../../vignettes/georobModelBuilding.tex", outputEncoding="macintosh")
% tools::Rd2latex("S3methods.georob.Rd", "../../../vignettes/S3methods.georob.tex", outputEncoding="macintosh")
% tools::Rd2latex("plot.georob.Rd", "../../../vignettes/plot.georob.tex", outputEncoding="macintosh")
% tools::Rd2latex("profilelogLik.Rd", "../../../vignettes/profilelogLik.tex", outputEncoding="macintosh")
% tools::Rd2latex("cv.georob.Rd", "../../../vignettes/cv.georob.tex", outputEncoding="macintosh")
...
with heteroscedastic nugget effect \eqn{\widehat{\tau}^2/\mbox{\boldmath
$w$\unboldmath}}{}, where \eqn{\mbox{\boldmath $w$\unboldmath}}{} are
the ``robustness weights'' \deqn{w_{i} =
...
\bibliographystyle{natbib}\bibliography{georob}
