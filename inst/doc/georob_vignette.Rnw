...
% <<meuse-zinc-load,eval=TRUE,echo=FALSE>>=
...
% \eqn{\xi=\sigma^2/\sigma_Z^2}{}, \eqn{\eta = \sigma_Z^2 /
...
signal process $Z(\mb{s})$ is the sum of several independent
...
($\sigma^2_{Z,k}$, $\mb{\theta}_{k}$), see equation
\ref{eq:covariance-matrix-signal}.
%
...
strctures but these variances are summed-up and assigned to the first
